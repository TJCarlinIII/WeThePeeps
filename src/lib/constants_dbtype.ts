...
export type SubjectStatus = "pending" | "completed" | "stonewalled" | "active" | "under_review" | "former" | "verified" | "deceased";
...
/**
 * UPDATED: SchemaField now includes 'boolean' and 'multi-relation' 
 * to support the new Join-Table architecture.
 */
export interface SchemaField {
  name: string;
  label: string;
  type: 'text' | 'textarea' | 'select' | 'relation' | 'date' | 'number' | 'boolean' | 'multi-relation';
  placeholder?: string;
  table?: string;
  options?: string[];
  required?: boolean;
  dependsOn?: string; // Support for filtered dropdowns
  helpText?: string;  // For TBI/Amnesia-friendly tooltips
}
 ...
 * These are kept exactly as they are so your current admin panel 
