...
export interface Incident {
  ...
  seo_keywords?: string;
  moral_violation_type?: string; // ✅ NEW: Moral Violation Type field
}
...
export default function IncidentForm({
  ...
  const emptyState: Incident = {
    ...
    seo_description: "",
    seo_keywords: "",
    moral_violation_type: "" // ✅ NEW: Default empty
  };
  ...
  // ✅ Moral Violation Type options
  const moralViolationOptions = [
    { value: "", label: "-- Select Moral Principle --" },
    { value: "not-kill", label: "Thou Shalt Not Kill (Medical Negligence)" },
    { value: "false-witness", label: "Thou Shalt Not Bear False Witness (Falsified Records)" },
    { value: "not-steal", label: "Thou Shalt Not Steal (Financial Exploitation)" },
    { value: "not-covet", label: "Thou Shalt Not Covet (Profit-Over-Patient)" },
    { value: "honor-parents", label: "Honor Thy Father and Mother (Elder/Vulnerable Abuse)" }
  ];
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {/* ✅ NEW: Moral Violation Type Dropdown */}
      <div>
        <label className="block text-[9px] text-slate-500 uppercase font-bold tracking-tighter mb-1">
          Moral_Violation_Type
        </label>
        <select
          value={formData.moral_violation_type || ''}
          onChange={(e) => setFormData({ ...formData, moral_violation_type: e.target.value })}
          className="w-full bg-black border border-slate-800 p-2 text-xs text-white focus:border-blue-500 outline-none font-mono"
        >
          {moralViolationOptions.map(opt => (
