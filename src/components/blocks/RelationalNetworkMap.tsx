...
/**
 * FILE: src/components/blocks/RelationalNetworkMap.tsx
 *
 * FIXES APPLIED:
 * 1. Standardized all relation lookups from rel.actor_id / rel.entity_id
 *    to rel.source_id / rel.target_id — matching the ActorEntityRelation interface
 *    in database-types.ts and the connections table schema.
 * 2. isActorFaded now resolves correctly against source_id.
 * 3. Connection lines render correctly since they look up source_id + target_id.
 */

import React, { useState, useMemo } from "react";
import { useRouter } from "next/navigation";
import type { EntityNode, ActorNode, ActorEntityRelation } from "@/lib/database-types";
...
export default function RelationalNetworkMap({
  entities,
  actors,
  relations,
  onEntitySelect,
  ...
  const entityPositions = useMemo(() => {
    ...
    entities.forEach((e, idx) => {
      const angle = (idx / Math.max(entities.length, 1)) * Math.PI * 2 - Math.PI / 2;
      pos.set(e.id, {
        x: cx + Math.cos(angle) * entityRadius,
        y: cy + Math.sin(angle) * entityRadius,
      });
    });
    return pos;
  }, [entities, cx, cy]);
  ...
  const actorPositions = useMemo(() => {
    ...
    actors.forEach((a, idx) => {
      const angle = (idx / Math.max(actors.length, 1)) * Math.PI * 2 - Math.PI / 2;
      pos.set(a.id, {
        x: cx + Math.cos(angle) * actorRadius,
        y: cy + Math.sin(angle) * actorRadius,
      });
    });
    return pos;
  }, [actors, cx, cy]);
  ...
  const isEntityFaded = (entityId: number) => {
    if (hoveredEntity !== null) return hoveredEntity !== entityId;
    if (selectedEntityId !== null) return selectedEntityId !== entityId;
    return false;
  };

  const isActorFaded = (actorId: number) => {
    if (hoveredActor !== null) return hoveredActor !== actorId;
    const targetEntityId = hoveredEntity ?? selectedEntityId;
    if (targetEntityId !== null) {
      // source_id is the actor, target_id is the entity
      const isConnected = relations.some(
        (r) => r.source_id === actorId && r.target_id === targetEntityId
      );
      return !isConnected;
    }
    return false;
  ...
  return (
    <div className="w-full aspect-[4/3] bg-[#0B1021] border border-slate-800 relative overflow-hidden flex items-center justify-center">
      <svg width="100%" height="100%" viewBox="0 0 800 600" className="opacity-90">
        {/* Radar rings */}
        <circle
          cx={cx} cy={cy} r={entityRadius}
          fill="none" stroke="#1E293B" strokeWidth="1" strokeDasharray="4 4"
        />
        <circle
          cx={cx} cy={cy} r={actorRadius}
          fill="none" stroke="#1E293B" strokeWidth="1" strokeDasharray="4 4"
        />

        {/* Connection lines (render below nodes) */}
        {relations.map((rel, idx) => {
          // source_id = actor, target_id = entity
          const aPos = actorPositions.get(rel.source_id);
          const ePos = entityPositions.get(rel.target_id);
          if (!aPos || !ePos) return null;

          const active = isLineActive(rel.source_id, rel.target_id);
          const faded =
            (hoveredActor !== null || hoveredEntity !== null || selectedEntityId !== null) &&
            !active;
          ...
          return (
            <path
              ...
              className={`transition-all duration-300 ${
                faded ? "opacity-10" : active ? "opacity-80" : "opacity-40"
              }`}
            />
          );
        })}

        {/* Entities — outer orbit */}
        {entities.map((ent) => {
          ...
          return (
            ...
            >
              <circle
                ...
                stroke={color}
                strokeWidth="1"
                strokeDasharray={isHighPriority ? "" : "2 4"}
                className={isHighPriority ? "animate-[spin_15s_linear_infinite]" : ""}
              />
              ...
              >
                {ent.name.length > 22 ? ent.name.slice(0, 22) + "…" : ent.name}
              </text>
            </g>
          );
        })}
          ...
          return (
            <g
              ...
              className={`cursor-pointer transition-all duration-300 ${
                isFaded ? "opacity-10" : "opacity-100"
              }`}
            >
              <circle
                r={isHovered ? "18" : "12"}
              ...
              >
                {actor.full_name.split(" ")[0].charAt(0)}
                {actor.full_name.split(" ").pop()?.charAt(0) ?? ""}
              </text>
              <text
                y="-20"
                fill="white"
