...
export default async function SubjectProfile({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const actor = await env.DB.prepare(
    "SELECT a.*, e.name as agency_name FROM actors a LEFT JOIN entities e ON a.entity_id = e.id WHERE a.slug = ? OR a.full_name = ?"
  ).bind(decodedSlug, decodedSlug).first<ActorProfile>();

  if (!actor) return notFound();

  const { results: evidence } = await env.DB.prepare(
    "SELECT id, title, content, created_at, isCritical FROM evidence WHERE official = ? ORDER BY created_at DESC"
