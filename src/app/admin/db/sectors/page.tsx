...
export default function SectorsManager() {
  ...
  const handleSave = async (formData: Sector) => {
    ...
    // Logic to auto-generate slug if it's a new entry and API expects it
    const payload = isEditing 
      ? { ...formData, id: selectedId ?? undefined } 
      : formData;
    ...
    try {
      ...
      if (res.ok) {
        alert("SECTOR_DATABASE_SYNCHRONIZED");
  ...
  return (
    <AdminGuard>
      <div className="min-h-screen bg-black text-white p-8 font-mono">
        ...
        <div className="mt-8 border border-slate-700 p-6 rounded-lg bg-slate-900/20 shadow-[0_0_15px_rgba(0,0,0,0.5)]">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-[#4A90E2] uppercase tracking-[0.2em] text-xs font-bold">
              {isEditing ? `SYSTEM_OVERRIDE [ID: ${selectedId}]` : 'DATA_INGRESS_SECTORS'}
            </h2>
            {isEditing && (
              <button 
        ...
        <div className="mt-12">
          <h3 className="text-slate-400 text-[10px] uppercase mb-4 tracking-widest font-bold">Sector_Manifest</h3>
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="bg-slate-900/40 border-b border-slate-700 text-slate-500 text-[9px] uppercase">
                  ...
                  <th className="p-4">Sector_Name</th>
