...
export default function StatutesManager() {
  ...
  useEffect(() => {
    fetch('/api/statutes')
      .then(res => res.json() as Promise<ApiResponse<Statute> | Statute[]>)
      .then((data) => {
  ...
  const handleSave = async (formData: Statute) => {
    ...
    try {
      ...
      if (res.ok) {
        alert("STATUTE_DATABASE_UPDATED");
  ...
  return (
    <AdminGuard>
      <div className="min-h-screen bg-black text-white p-8 font-mono">
        ...
        <div className="mt-8 border border-slate-700 p-6 rounded-lg bg-slate-900/20 shadow-[0_0_15px_rgba(0,0,0,0.5)]">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-[#4A90E2] uppercase tracking-[0.2em] text-xs font-bold">
              {isEditing ? `LEGAL_OVERRIDE [ID: ${selectedId}]` : 'DATA_INGRESS_STATUTES'}
            </h2>
            {isEditing && (
              <button 
        ...
        <div className="mt-12">
          <h3 className="text-slate-400 text-[10px] uppercase mb-4 tracking-widest font-bold">Statute_Manifest</h3>
              ...
              <tbody>
                {rows.map(row => (
                  ...
                  >
                    ...
                    <td className="p-4 font-mono text-[#4A90E2] font-bold">{row.citation}</td>
                    <td className={`p-4 ${selectedId === row.id ? "text-white" : "text-slate-400"}`}>{row.title}</td>
                    <td className="p-4"><span className="text-[9px] border border-slate-700 px-2 py-0.5 rounded">{row.jurisdiction}</span></td>
