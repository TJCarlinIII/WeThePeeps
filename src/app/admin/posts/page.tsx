...
// Updated imports to use the new dynamic handlers
import { getRecords, deleteRecord } from "@/app/admin/actions";
import AdminSidebar from "@/components/admin/AdminSidebar";
import AdminGuard from "@/components/admin/AdminGuard";
import { slugify } from "@/lib/stringutils";

// Updated to match D1 table names and lowercase for consistency
...
export default function AdminPostsPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      // Corrected to use dynamic getRecords
  ...
  useEffect(() => {
    const initializePage = async () => {
      try {
        ...
        if (data.terms) {
          setCategories(data.terms.filter((t: TaxonomyTerm) => t.type === 'category'));
        }
      } catch (err) {
        console.error("INITIALIZATION_ERROR", err);
        setStatus("Error: System_Initialization_Failed");
      }
    };
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (result.success) {
      ...
      setStatus(`Error: ${result.error || "Sync_Failed"}`);
    }
  }

  const handlePurge = async (id: number) => {
    if (window.confirm("CRITICAL_PURGE: Are you sure?")) {
      // Updated to use the dynamic deleteRecord(table, id)
  ...
  const inputClass = "bg-slate-950 border border-slate-800 p-3 text-sm text-white focus:border-[#4A90E2] outline-none transition-colors w-full";
        ...
        <main className="flex-1 p-8 overflow-y-auto">
          <div className="max-w-5xl mx-auto">
            <header className="mb-12 border-b border-slate-900 pb-6 flex justify-between items-end">
              <div>
                <h1 className="text-3xl font-black italic tracking-tighter uppercase text-[#4A90E2]">Intelligence_Dispatch</h1>
            ...
            {loading ? (
              <div className="animate-pulse text-[#4A90E2] tracking-[0.3em] py-10">INITIALIZING_SCAN...</div>
            ) : (
              <div className="mb-16">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-[#4A90E2] text-xs font-black uppercase tracking-[0.3em]">
                    {viewMode === 'media' ? 'Evidence_Archive' : 'Post_Registry'} ({records.length})
                  </h2>
                  <button
                    onClick={() => loadData()}
                ...
                <div className="grid grid-cols-1 gap-4">
                    ...
                    <div
                      ...
                      className={`border p-6 rounded-lg transition-all ${
                        editingId === item.id
                          ...
                          : 'border-slate-800 bg-slate-900/10 hover:border-slate-600'
                      }`}
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex-1">
                          {item.category && (
                            <span className="text-[9px] text-[#4A90E2] uppercase font-bold tracking-widest mb-1 block">
                              [{item.category}]
                            </span>
                          )}
                        ...
                        <div className="flex gap-2">
                          {viewMode === 'posts' && (
                            <button
                              ...
                              className="text-[9px] font-black px-3 py-1 border border-slate-800 hover:bg-white hover:text-black transition-all uppercase"
                            >
                              Edit
                            </button>
                          )}
                  ...
                  {records.length === 0 && (
                    <div className="text-center py-20 border border-dashed border-slate-800 text-slate-600 uppercase text-xs">
                      No records found.
                    </div>
                  )}
                </div>
            ...
            {viewMode === 'posts' && (
              <section id="post-form-section" className="border border-[#4A90E2]/30 p-8 bg-slate-900/10 rounded-lg">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-[#4A90E2] text-xs font-black uppercase tracking-[0.3em]">
                  ...
                  {editingId && (
                    <button
                      type="button"
                      onClick={resetForm}
                      className="text-[9px] text-slate-500 uppercase hover:text-white underline"
                    >
                      [ ABORT_EDIT ]
                    </button>
                  )}
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <input
                        required
                        className={`${inputClass} border-dashed text-[#4A90E2]`}
                        value={slug}
                  ...
                  <div className="flex items-center gap-4 py-4 border-y border-slate-900">
                    ...
                    >
                      Promote_To_Primary_Matrix (Featured)
                    </label>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <button
                      type="submit"
                      className="bg-[#4A90E2] text-black font-black uppercase px-10 py-4 italic tracking-tighter hover:bg-white transition-all"
                    >
