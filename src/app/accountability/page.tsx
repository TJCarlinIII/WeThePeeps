...
// This would eventually come from your D1 Database
const subjects = [
  { name: "Pat McRae", role: "Township Supervisor", status: "Active Investigation", agency: "Redford Administration" },
  { name: "Jennifer Mansfield", role: "Police Chief", status: "Criminal Complaint Filed", agency: "Redford PD" },
  { name: "Karla M. Sanders", role: "Clerk / FOIA Coordinator", status: "Records Dispute", agency: "Redford Clerk" },
  { name: "Michael Bosnic", role: "Contract Prosecutor", status: "Conflict of Interest", agency: "GMN Law" },
  // ... more subjects
];
    ...
    <div className="min-h-screen bg-black text-white p-8 font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {subjects.map((subject) => (
          <a 
            key={subject.name}
            href={`/accountability/${subject.name.toLowerCase().replace(/ /g, '-')}`}
          ...
          >
            ...
            <div className="space-y-2 border-t border-slate-800 pt-4">
              <div className="flex justify-between text-[10px] uppercase">
                ...
                <span className="text-slate-300">{subject.role}</span>
              </div>
              <div className="flex justify-between text-[10px] uppercase">
                <span className="text-slate-500">Alert_Level:</span>
                <span className={cn(
                  "font-bold",
                  subject.status.includes("Criminal") ? "text-red-500" : "text-yellow-500"
                )}>
                  {subject.status}
                </span>
              </div>
